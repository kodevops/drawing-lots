...
=end
  ...
  config.after(:each, :type => :feature) do |example|
    # Save screenshot and HTML when the test fails
    if example.exception && page.present?
      meta = example.metadata
      name = "feature-#{File.basename(meta[:file_path])}-line:#{meta[:line_number]}"
      file_path = ["screenshots", name].join('/')

      page.save_screenshot("#{file_path}.png", full: true)
    end
  end
end
